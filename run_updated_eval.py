...
print("=== 3. Cleaning & Parsing Functions with Syllable Repair ===")

def clean_syllables(text):
    preps = {'в', 'на', 'за', 'под', 'над', 'от', 'до', 'по', 'о', 'об', 'обо', 'без', 'для', 'при', 'не', 'ни', 'то', 'как', 'так', 'же', 'ли', 'бы', 'он', 'она', 'они', 'мы', 'вы', 'я', 'ты', 'или', 'что', 'кто', 'где', 'чем', 'кем'}
            ...
            while changed:
                ...
                while i < len(tokens):
                    if i + 1 < len(tokens):
                        ...
                        if len(w1) <= 4 and len(w2) <= 5 and re.match(r'^[а-яёА-ЯЁ]+$', w1) and re.match(r'^[а-яёА-ЯЁ]+$', w2):
                            if (w1.lower() not in preps or len(w2) <= 3) and (w2.lower() not in preps):
