...
def extract_pdf_data(raw_entries):
    if not raw_entries:
        return []
    ...
    # Split by numbered senses or POS
...
def calculate_coincidence_score(dict_meanings, ideal_meanings):
    """
    Calculates coincidence percentage (0.0 - 100.0%)
    Weights:
      1. Primary meaning (#1 priority) alignment: 45%
      2. Ideal set coverage / recall: 40%
      3. Order & quality / conciseness: 15%
    """
    ...
    i1 = ideal_meanings[0] # Top 1 core ideal sense
    d1 = dict_meanings[0] # Dictionary's 1st sense
    
    # 1. Primary (#1) Match Analysis (W1 = up to 45 pts)
    w1_score = 0.0
    d1_matches_i1 = meanings_match(d1, i1)
    d1_matches_any_ideal = any(meanings_match(d1, im) for im in ideal_meanings)
    
    # Check if i1 appears anywhere in dict_meanings (e.g. position 2 or 3)
    i1_pos_in_dict = -1
    ...
    if d1_matches_i1:
        w1_score = 45.0 # Direct perfect match for top sense
    elif d1_matches_any_ideal:
        # 1st sense is in ideal set, but not #1
        w1_score = 25.0
        if i1_pos_in_dict == 1: # i1 is at position 2
            w1_score += 15.0 # total 40.0
        elif i1_pos_in_dict > 1:
            w1_score += 10.0
    elif i1_pos_in_dict == 1:
        # 1st sense wasn't ideal, but i1 is right at #2
        w1_score = 22.0
    elif i1_pos_in_dict > 1:
        w1_score = 15.0
    else:
    ...
    # 2. Coverage of Ideal Meanings (W2 = up to 40 pts)
    matched_ideals = set()
    ...
    # 3. Order, Conciseness & Noise (W3 = up to 15 pts)
    w3_score = 0.0
    # Conciseness: not overloaded with junk
    if len(dict_meanings) <= num_ideal + 1 and coverage_ratio >= 0.8:
        w3_score += 8.0
    elif len(dict_meanings) <= num_ideal + 3:
        w3_score += 4.0
        
    # Order preservation: ideal matches appear in top positions
...
for idx, word_obj in enumerate(sampled_1000):
    ...
    # 1. Extract Native Data from each Dictionary
    ...
    muller_300k_data = extract_pdf_data(pdf_indices['muller_300k'].get(word, []))
    ...
    # 2. Determine Gold Standard Ideal Meanings
    # Clean core meanings from Oxford + CLD + Sam
    ideal_list = []
    # Primary #1: Oxford #1 or CLD #1
    if cld_data:
        ideal_list.append(cld_data[0])
    elif ox_data:
        ideal_list.append(ox_data[0])
        
    # Additional major POS / senses
    ...
    # 3. Calculate Coincidence Object for Each Dictionary
