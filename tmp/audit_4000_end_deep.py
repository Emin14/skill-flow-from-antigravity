...
for idx_rel, entry in enumerate(entries_4000_end):
    idx_abs = 4000 + idx_rel
    word = entry['word']
...
out_txt = io.open('tmp/audit_4000_end_summary.txt', 'w', encoding='utf-8')
out_txt.write(f"FULL AUDIT SUMMARY FOR WORDS 4001-4982 (Found {len(report)} words with issues)\n")
out_txt.write("="*70 + "\n\n")
...
out_json = io.open('tmp/audit_4000_end_full_report.json', 'w', encoding='utf-8')
json.dump(report, out_json, ensure_ascii=False, indent=2)
out_json.close()

print(f"Audit 4000-end completed: {len(report)} words with issues.")
for t, cnt in counts.most_common():
    print(f"  {t}: {cnt}")
