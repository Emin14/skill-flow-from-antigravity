...
entries_3500_4000 = data[3500:4000]
...
for idx_rel, entry in enumerate(entries_3500_4000):
    idx_abs = 3500 + idx_rel
    word = entry['word']
    ...
    for m in entry.get('meanings', []):
        ...
        # 8. Check examples for OCR errors or ≅
        for eidx, ex in enumerate(m.get('examples', [])):
            ...
            if '≅' in ru or '♦' in ru:
                issues.append({
                    ...
                    'detail': 'Example contains idiom symbol ≅ or ♦'
                })
            for pat, desc in ocr_checks:
                if re.search(pat, ru):
                    issues.append({
...
out_txt = io.open('tmp/audit_3500_4000_summary.txt', 'w', encoding='utf-8')
out_txt.write(f"FULL AUDIT SUMMARY FOR WORDS 3501-4000 (Found {len(report)} words with issues)\n")
out_txt.write("="*70 + "\n\n")
...
out_json = io.open('tmp/audit_3500_4000_full_report.json', 'w', encoding='utf-8')
json.dump(report, out_json, ensure_ascii=False, indent=2)
out_json.close()

print(f"Audit 3500-4000 completed: {len(report)} words with issues.")
for t, cnt in counts.most_common():
    print(f"  {t}: {cnt}")
