...
entries_1000_1500 = data[1000:1500]
...
for idx_rel, entry in enumerate(entries_1000_1500):
    idx_abs = 1000 + idx_rel
    word = entry['word']
...
out_txt = io.open('tmp/audit_1000_1500_summary.txt', 'w', encoding='utf-8')
out_txt.write(f"FULL AUDIT SUMMARY FOR WORDS 1001-1500 (Found {len(report)} words with issues)\n")
out_txt.write("="*70 + "\n\n")
...
out_json = io.open('tmp/audit_1000_1500_full_report.json', 'w', encoding='utf-8')
json.dump(report, out_json, ensure_ascii=False, indent=2)
out_json.close()

print(f"Audit 1000-1500 completed: {len(report)} words with issues.")
for t, cnt in counts.most_common():
    print(f"  {t}: {cnt}")
