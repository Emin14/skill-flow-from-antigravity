...
entries_2500_3000 = data[2500:3000]
...
for idx_rel, entry in enumerate(entries_2500_3000):
    idx_abs = 2500 + idx_rel
    word = entry['word']
...
out_txt = io.open('tmp/audit_2500_3000_summary.txt', 'w', encoding='utf-8')
out_txt.write(f"FULL AUDIT SUMMARY FOR WORDS 2501-3000 (Found {len(report)} words with issues)\n")
out_txt.write("="*70 + "\n\n")
...
out_json = io.open('tmp/audit_2500_3000_full_report.json', 'w', encoding='utf-8')
json.dump(report, out_json, ensure_ascii=False, indent=2)
out_json.close()

print(f"Audit 2500-3000 completed: {len(report)} words with issues.")
for t, cnt in counts.most_common():
    print(f"  {t}: {cnt}")
