...
entries_3000_3500 = data[3000:3500]
...
for idx_rel, entry in enumerate(entries_3000_3500):
    idx_abs = 3000 + idx_rel
    word = entry['word']
...
out_txt = io.open('tmp/audit_3000_3500_summary.txt', 'w', encoding='utf-8')
out_txt.write(f"FULL AUDIT SUMMARY FOR WORDS 3001-3500 (Found {len(report)} words with issues)\n")
out_txt.write("="*70 + "\n\n")
...
out_json = io.open('tmp/audit_3000_3500_full_report.json', 'w', encoding='utf-8')
json.dump(report, out_json, ensure_ascii=False, indent=2)
out_json.close()

print(f"Audit 3000-3500 completed: {len(report)} words with issues.")
for t, cnt in counts.most_common():
    print(f"  {t}: {cnt}")
