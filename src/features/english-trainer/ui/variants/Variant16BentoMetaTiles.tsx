...
/**
 * Variant 16: Editorial Serif Headline with Floating Superscript Sound
 */
export const Variant16BentoMetaTiles: React.FC<BaseWordCardProps> = ({
  currentCard,
  meaningsList,
  safeMeaningIndex,
  currentMeaning,
  ...
  const renderFormsRow = () => {
    if (forms.verbForms && (forms.verbForms.past || forms.verbForms.pastParticiple || forms.verbForms.ing)) {
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#09090b', fontWeight: 600 }}>
          {forms.verbForms.past && <span><strong style={{ color: '#854d0e', fontWeight: 800 }}>past:</strong> {forms.verbForms.past}</span>}
          {forms.verbForms.past && forms.verbForms.pastParticiple && <span style={{ color: '#e2e8f0' }}>|</span>}
          {forms.verbForms.pastParticiple && <span><strong style={{ color: '#854d0e', fontWeight: 800 }}>part.:</strong> {forms.verbForms.pastParticiple}</span>}
          {(forms.verbForms.past || forms.verbForms.pastParticiple) && forms.verbForms.ing && <span style={{ color: '#e2e8f0' }}>|</span>}
          {forms.verbForms.ing && <span><strong style={{ color: '#854d0e', fontWeight: 800 }}>-ing:</strong> {forms.verbForms.ing}</span>}
        </div>
      );
    }
    if (forms.nounForms?.plural) {
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#09090b', fontWeight: 600 }}>
          <span><strong style={{ color: '#854d0e', fontWeight: 800 }}>pl.:</strong> {forms.nounForms.plural}</span>
        </div>
      );
    }
    if (forms.adjectiveForms?.comparative || forms.adjectiveForms?.superlative) {
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#09090b', fontWeight: 600 }}>
          {forms.adjectiveForms.comparative && <span><strong style={{ color: '#854d0e', fontWeight: 800 }}>comp.:</strong> {forms.adjectiveForms.comparative}</span>}
          {forms.adjectiveForms.comparative && forms.adjectiveForms.superlative && <span style={{ color: '#e2e8f0' }}>|</span>}
          {forms.adjectiveForms.superlative && <span><strong style={{ color: '#854d0e', fontWeight: 800 }}>superl.:</strong> {forms.adjectiveForms.superlative}</span>}
        </div>
      );
    }
    return null;
  ...
  return (
    ...
    >
      {/* 1. Headword Header (Variation 16: Editorial Serif Headline with Floating Superscript Sound) */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '4px' }}>
          <span style={{ fontSize: '32px', fontWeight: 700, color: '#1c1917', fontFamily: 'Georgia, serif', letterSpacing: '-0.3px', lineHeight: 1 }}>
            {currentCard.word}
          </span>
          <button
            type="button"
        ...
        <span style={{ fontSize: '15px', color: '#78716c', fontFamily: 'Georgia, serif', fontStyle: 'italic' }}>
          /{displayTranscription}/
        </span>
      </div>
      ...
      <div
        style={{
          ...
          gap: '6px',
          boxShadow: '0 4px 14px rgba(120,113,108,0.04)',
          height: '195px',
          minHeight: '195px',
          maxHeight: '195px',
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', overflowY: 'auto', scrollbarWidth: 'none' }}>
          {/* Header: Translation + Part of Speech + Tight Register Badge */}
          <div>
            {currentMeaning.register && currentMeaning.register.length > 0 && (
              <div style={{ display: 'flex', gap: '3px', alignItems: 'center', marginBottom: '-1px' }}>
                {currentMeaning.register.map((reg, idx) => (
                  <span
                    key={idx}
                    style={{
                      background: '#f5efe6',
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: '16px', fontWeight: 800, color: '#1c1917', lineHeight: 1.2 }}>
          ...
          {currentMeaning.examples && currentMeaning.examples.length > 0 && (
            <div style={{ height: '1px', background: '#f5efe6', margin: '2px 0 2px 0' }} />
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {currentMeaning.examples.slice(0, 2).map((ex, i) => (
                <div key={i} style={{ fontSize: '12px', lineHeight: 1.35 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: 'transparent',
                        cursor: 'pointer',
                        color: copiedIndex === i ? '#16a34a' : '#a8a29e',
                        padding: '1px',
                      }}
                      title="Скопировать"
                    >
                  ...
                  {ex.ru && (
                    <div style={{ color: '#854d0e', fontSize: '11px', paddingLeft: '10px', marginTop: '1px' }}>
                      {ex.ru}
                    </div>
                  )}
                </div>
          ...
          {currentMeaning.synonyms && currentMeaning.synonyms.length > 0 && (
            <div style={{ fontSize: '11.5px', marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              <span style={{ color: '#854d0e', fontWeight: 800 }}>Синонимы: </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '4px', borderTop: '1px solid #f5efe6' }}>
          <button
            type="button"
            disabled={safeMeaningIndex === 0}
            onClick={() => { if (safeMeaningIndex > 0) onSelectMeaning(safeMeaningIndex - 1); }}
            style={{
              width: '24px',
              height: '24px',
              borderRadius: '6px',
            ...
            {meaningsList.map((_, i) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: '50%',
                  background: i === safeMeaningIndex ? '#1c1917' : '#d6d3d1',
                  border: 'none',
                  padding: 0,
                  cursor: 'pointer',
          ...
          <button
            ...
            onClick={() => { if (safeMeaningIndex < total - 1) onSelectMeaning(safeMeaningIndex + 1); }}
            style={{
              width: '24px',
              height: '24px',
              borderRadius: '6px',
