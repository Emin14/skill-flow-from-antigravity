...
const DATE_PATTERNS_DESCRIPTIONS: Record<number, { name: string; rationale: string }> = {
  1: {
    name: '1. Стандартный выпадающий список',
    rationale: 'Нативный <select>. Простой и привычный. Текущая реализация без изменений.',
  },
  2: {
    name: '2. Умный дропдаун → сразу открывает календарь',
    rationale: 'Select с 4 опциями. При выборе «Выбрать дату...» — сразу открывается нативный календарь. Дата отображается прямо в поле. 0px сдвига.',
  },
  3: {
    name: '3. Glassmorphic pill — стеклянная плашка',
    rationale: 'Одна кнопка с эффектом стекла и свечения. Всплывающее меню с backdrop-blur поверх. Современный «жидкий» стиль.',
  },
  4: {
    name: '4. Неоновые чипы — светящиеся кнопки',
    rationale: 'Три чипа с неоновым свечением. Активный чип горит ярче. Текущая дата светится как активная кнопка.',
  },
  5: {
    name: '5. Минималистичный underline — линия снизу',
    rationale: 'Текстовая кнопка без рамки, только подчёркивание снизу. Меню появляется как лёгкая тень. Ультраминимализм.',
  },
  6: {
    name: '6. Иконки-действия — компактный icon row',
    rationale: 'Ряд из 4 круглых кнопок-иконок. Активная иконка подсвечивается. Экономит место, выглядит как тулбар.',
  },
  7: {
    name: '7. Frosted card — карточка из матового стекла',
    rationale: 'Кнопка в стиле Apple Card с матовым фоном. Меню появляется как карточка с blur. Элегантный iOS-стиль.',
  },
  8: {
    name: '8. Gradient badge — градиентный значок',
    rationale: 'Кнопка с анимированным градиентом (фиолет → синий). Всплывает плашка с цветными вариантами. Яркий акцентный стиль.',
  },
  9: {
    name: '9. Outline chips — рамочные чипы',
    rationale: 'Прозрачные чипы с только рамкой. Активный заполняется. Лаконичный стиль Material You.',
  },
  10: {
    name: '10. Amber HUD — янтарное командное меню',
    rationale: 'Плашка с янтарным акцентом и монospace-шрифтом. Выпадает терминал-стиль меню. Уникальный «командный» характер.',
  },
};
...
export const EditTaskModal: React.FC<EditTaskModalProps> = ({ task, isOpen, onClose, onSaveSuccess }) => {
  ...
  const renderSingleDateControl = () => {
    ...
    // Variant 4: Neon Chips — светящиеся чипы
    if (dateVariant === 4) {
      const neonChips = [
      ...
      return (
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          {neonChips.map(({ label, full, mode, action, color, glow }) => {
            ...
            return (
              <button key={mode} type="button" onClick={action} style={{
                flex: mode === 'custom' ? 1 : 'none',
                height: '38px', borderRadius: '10px', padding: '0 10px',
                border: `1.5px solid ${isActive ? color : 'rgba(255,255,255,0.12)'}`,
                background: isActive ? `rgba(${color === '#facc15' ? '250,204,21' : color === '#fb923c' ? '251,146,60' : '52,211,153'},0.15)` : 'transparent',
          ...
          <button type="button" onClick={selectNone} style={{
            height: '38px', width: '34px', borderRadius: '10px',
            border: '1.5px solid rgba(248,113,113,0.3)',
            background: datePresetMode === 'none' ? 'rgba(248,113,113,0.15)' : 'transparent',
    ...
    // Variant 5: Minimal Underline — ультра-минималистичный
    if (dateVariant === 5) {
      return (
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              padding: '0 4px', cursor: 'pointer', transition: 'border-color 0.2s',
            }}
            onFocus={e => (e.currentTarget.style.borderBottomColor = 'rgba(99,102,241,0.8)')}
            onBlur={e => (e.currentTarget.style.borderBottomColor = 'rgba(255,255,255,0.2)')}
          >
            <span style={{ letterSpacing: '0.01em' }}>{getDateStatusLabel()}</span>
          ...
          {activePopover && (
            <div style={{
              ...
              boxShadow: '0 8px 32px rgba(0,0,0,0.7)', border: '1px solid rgba(255,255,255,0.08)',
              display: 'flex', flexDirection: 'column',
            }}>
              {[
                { label: 'Сегодня', sub: 'сб', action: selectToday },
                { label: 'Завтра', sub: 'вс', action: selectTomorrow },
                { label: 'Выбрать дату', sub: '...', action: triggerHiddenPicker },
                { label: 'Без даты', sub: '—', action: selectNone },
              ].map(({ label, sub, action }) => (
                <button key={label} type="button" onClick={action} style={{
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  background: 'transparent', border: 'none', borderRadius: '6px',
                  color: 'var(--color-text)', fontSize: '13px', padding: '7px 10px',
                  cursor: 'pointer',
                }}>
                  <span>{label}</span>
                  <span style={{ opacity: 0.3, fontSize: '11px' }}>{sub}</span>
                </button>
              ))}
            </div>
          )}
    ...
    // Variant 6: Icon Row — компактный ряд круглых иконок
    if (dateVariant === 6) {
      const icons = [
      ...
      return (
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {icons.map(({ emoji, label, mode, action }) => {
            const isActive = datePresetMode === mode;
            return (
              <button key={mode} type="button" onClick={action}
                title={label}
                style={{
                  flexShrink: 0, width: '40px', height: '40px', borderRadius: '50%',
                  background: isActive ? 'rgba(99,102,241,0.25)' : 'rgba(255,255,255,0.05)',
                  border: isActive ? '2px solid #818cf8' : '2px solid rgba(255,255,255,0.1)',
          ...
          {datePresetMode === 'custom' && scheduledDate && (
            <span style={{ fontSize: '12px', color: '#818cf8', fontWeight: 600, flex: 1 }}>
              {formatDateDisplay(scheduledDate)}
            </span>
          )}
        </div>
    ...
    // Variant 7: Frosted Card — матовое стекло в стиле Apple
    if (dateVariant === 7) {
      return (
        <div style={{ position: 'relative' }}>
          <button
          ...
          {activePopover && (
            <div style={{
              position: 'absolute', top: '46px', left: 0, right: 0, zIndex: 100,
              background: 'rgba(28,35,55,0.95)', backdropFilter: 'blur(24px)',
              border: '1px solid rgba(255,255,255,0.12)', borderRadius: '16px',
              overflow: 'hidden', boxShadow: '0 24px 48px rgba(0,0,0,0.7)',
            }}>
              {[
                { label: 'Сегодня', icon: '☀️', action: selectToday },
                { label: 'Завтра', icon: '🌅', action: selectTomorrow },
                { label: 'Выбрать дату...', icon: '📆', action: triggerHiddenPicker },
                { label: 'Без даты', icon: '✕', action: selectNone, destructive: true },
              ].map(({ label, icon, action, destructive }, i, arr) => (
                <button key={label} type="button" onClick={action} style={{
                  width: '100%', background: 'transparent',
                  border: 'none', borderTop: i > 0 ? '1px solid rgba(255,255,255,0.06)' : 'none',
                  color: destructive ? '#f87171' : 'var(--color-text)',
                  fontSize: '15px', padding: '13px 16px',
    ...
    // Variant 8: Gradient Badge — анимированный градиентный значок
    if (dateVariant === 8) {
      return (
        <div style={{ position: 'relative' }}>
          <button
          ...
          {activePopover && (
            <div style={{
              ...
              padding: '6px', boxShadow: '0 12px 40px rgba(139,92,246,0.3)',
              display: 'flex', flexDirection: 'column', gap: '3px',
            }}>
              {[
                { label: 'Сегодня', icon: '☀️', action: selectToday, color: '#a5b4fc' },
                { label: 'Завтра', icon: '🌅', action: selectTomorrow, color: '#93c5fd' },
                { label: 'Выбрать дату...', icon: '📆', action: triggerHiddenPicker, color: '#c4b5fd' },
                { label: 'Без даты', icon: '✕', action: selectNone, color: '#f87171' },
              ].map(({ label, icon, action, color }) => (
                <button key={label} type="button" onClick={action} style={{
                  background: 'rgba(255,255,255,0.04)', border: 'none', borderRadius: '8px',
                  color, fontSize: '13px', padding: '8px 12px', cursor: 'pointer',
    ...
    // Variant 9: Outline Chips — рамочные чипы Material You
    if (dateVariant === 9) {
      const outlineChips = [
          ...
          {outlineChips.map(({ label, mode, action }) => {
            ...
            return (
              <button key={mode} type="button" onClick={action} style={{
                ...
                border: `1.5px solid ${isActive ? 'var(--color-text)' : 'rgba(255,255,255,0.2)'}`,
                background: isActive ? 'var(--color-text)' : 'transparent',
                color: isActive ? 'var(--color-bg)' : 'rgba(255,255,255,0.65)',
    ...
    // Variant 10: Amber HUD — янтарное командное меню
    return (
      <div style={{ position: 'relative' }}>
        <button
          type="button"
        ...
        {activePopover && (
          <div style={{
            position: 'absolute', top: '46px', left: 0, right: 0, zIndex: 100,
            background: '#0c0a00', border: '1px solid rgba(217,119,6,0.6)',
            borderRadius: '8px', padding: '4px',
            boxShadow: '0 8px 32px rgba(217,119,6,0.2)',
            display: 'flex', flexDirection: 'column',
          }}>
            {[
              { key: 'T', label: 'Сегодня', action: selectToday },
              { key: 'TM', label: 'Завтра', action: selectTomorrow },
              { key: 'D', label: 'Выбрать дату...', action: triggerHiddenPicker },
              { key: 'X', label: 'Без даты', action: selectNone, dim: true },
            ].map(({ key, label, action, dim }) => (
              <button key={key} type="button" onClick={action} style={{
                background: 'transparent', border: 'none',
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                color: dim ? 'rgba(251,191,36,0.4)' : '#fbbf24',
                fontFamily: 'monospace', fontSize: '13px', padding: '7px 10px',
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <input
          ...
          style={{
            ...
            // Fix iOS dark mode: native date picker UI inherits color-scheme
            colorScheme: 'dark',
          }}
        />
