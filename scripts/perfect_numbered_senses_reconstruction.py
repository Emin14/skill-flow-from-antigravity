# -*- coding: utf-8 -*-
"""
Perfect root-preserving numbered senses reconstruction.
"""
import json, re, sys
...
# Strict dotted register dictionary
REGISTER_DICT = [
    (r'\bразг\.\s*', 'разговорное'),
    (r'\bпрост\.\s*', 'просторечное'),
    (r'\bкнижн\.\s*', 'книжное'),
    (r'\bпоэт\.\s*', 'поэтическое'),
    (r'\bпосл\.\s*', 'пословица'),
    (r'\bуст\.\s*', 'устаревшее'),
    (r'\bредк\.\s*', 'редкое'),
    (r'\bперен\.\s*', 'переносное'),
    (r'\bшутл\.\s*', 'шутливое'),
    (r'\bирон\.\s*', 'ироническое'),
    (r'\bсл\.\s*', 'сленг'),
    (r'\bамер\.\s*', 'американизм'),
    (r'\bбрит\.\s*', 'британское'),
    (r'\bтех\.\s*', 'техническое'),
    (r'\bвоен\.\s*', 'военное'),
    (r'\bмор\.\s*', 'морской термин'),
    (r'\bюр\.\s*', 'юридическое'),
    (r'\bмед\.\s*', 'медицинское'),
    (r'\bбиол\.\s*', 'биологическое'),
    (r'\bбот\.\s*', 'ботаника'),
    (r'\bзоол\.\s*', 'зоология'),
    (r'\bанат\.\s*', 'анатомия'),
    (r'\bэл\.\s*', 'электротехника'),
    (r'\bвчт\.\s*', 'вычислительная техника'),
    (r'\bмат\.\s*', 'математика'),
    (r'\bфиз\.\s*', 'физика'),
    (r'\bхим\.\s*', 'химия'),
    (r'\bгеол\.\s*', 'геология'),
    (r'\bэкон\.\s*', 'экономика'),
    (r'\bфин\.\s*', 'финансы'),
    (r'\bкомм\.\s*', 'коммерческое'),
    (r'\bком\.\s*', 'коммерческое'),
    (r'\bав\.\s*', 'авиация'),
    (r'\bрадио\b\s*', 'радио'),
    (r'\bмуз\.\s*', 'музыка'),
    (r'\bспорт\.\s*', 'спортивное'),
    (r'\bкул\.\s*', 'кулинария'),
    (r'\bархит\.\s*', 'архитектура'),
    (r'\bграм\.\s*', 'грамматика'),
    (r'\bлингв\.\s*', 'лингвистика'),
    (r'\bцерк\.\s*', 'церковное'),
    (r'\bфото\b\s*', 'фотография'),
    (r'\bбухг\.\s*', 'бухгалтерия'),
    (r'\bтеатр\.\s*', 'театр')
]

def clean_ocr(t):
    if not t: return ""
    # Glue hyphenated word across lines
    t = re.sub(r'([А-Яа-яA-Za-z])-\s*≅?\s*([а-яa-z])', r'\1\2', t)
    t = re.sub(r'([А-Яа-яA-Za-z])\xad\s*([а-яa-z])', r'\1\2', t)
    t = t.replace('≅', '').replace('\xad', '').replace('\u00ad', '').replace('\ue000', '')
    t = re.sub(r'\.\s+\.\s+\.', '...', t)
    # Remove accidental space inside syllables:
    # e.g. "сопро вождать" -> "сопровождать", "ак компанировать" -> "аккомпанировать", "обвине ние" -> "обвинение"
    ocr_word_fixes = [
        ('сопро вождать', 'сопровождать'),
        ('ак компанировать', 'аккомпанировать'),
        ('обвине ние', 'обвинение'),
        ('вдо бавок', 'вдобавок'),
        ('пере делывать', 'переделывать'),
        ('холо стить', 'холостить'),
        ('достижИмый', 'достижимый'),
        ('ширОкой', 'широкой'),
        ('широкий', 'широкий')
    ]
    for w_from, w_to in ocr_word_fixes:
        t = t.replace(w_from, w_to)
...
def parse_raw_meaning_content(chunk, base_word, default_pos, sense_id):
    ...
    # Extract register strictly at beginning with dot
    for reg_pat, reg_val in REGISTER_DICT:
        if re.match(rf'^{reg_pat}', chunk):
            if reg_val not in regs:
                regs.append(reg_val)
            chunk = re.sub(rf'^{reg_pat}', '', chunk).strip(' ;,')
    ...
    for it in items[1:]:
        ...
        if m_ex:
            ...
            ex_regs = []
            for reg_pat, reg_val in REGISTER_DICT:
                if re.match(rf'^{reg_pat}', ru_p):
                    ex_regs.append(reg_val)
                    ru_p = re.sub(rf'^{reg_pat}', '', ru_p).strip(' ;,')
            ex_obj = {
...
print("Applied perfect root-preserving numbered senses reconstruction!")
