# -*- coding: utf-8 -*-
"""
Clean reconstruction of numbered senses with root protection:
- Word boundary + dot check for abbreviations (protect 'бухгалтер', 'моря', 'собираться')
- Glue OCR split syllables ('сопро вождать' -> 'сопровождать')
- Clean leading punctuation ('. аккомпанировать' -> 'аккомпанировать')
"""
import json, re, sys
...
# Comprehensive register map with dots
REGISTER_MAP_DOTS = {
    'разг.': 'разговорное',
    'прост.': 'просторечное',
    'книжн.': 'книжное',
...
def parse_raw_meaning_content(chunk, base_word, default_pos, sense_id):
    ...
    for reg_k, reg_v in REGISTER_MAP_DOTS.items():
        pattern = rf'^{re.escape(reg_k)}\s*'
    ...
    for it in items[1:]:
        ...
        if m_ex:
            ...
            ex_regs = []
            for rk, rv in REGISTER_MAP_DOTS.items():
    ...
    # Clean leading dots, semicolons, dashes
    main_def = re.sub(r'^[.;,\-\s]+', '', main_def).strip()
    
    # Fix broken syllables inside words: e.g. "ак компанировать" -> "аккомпанировать", "обвине ние" -> "обвинение"
    # Specific dictionary fixes
    fixes = [
    ...
    for f_from, f_to in fixes:
        main_def = main_def.replace(f_from, f_to)
        
    for ex in examples:
        for f_from, f_to in fixes:
            ex['ru'] = ex['ru'].replace(f_from, f_to)
...
print("Applied clean numbered senses reconstruction!")
