...
# Join multi-line bracketed transcriptions
full_text = re.sub(r'\[[^\]]{1,120}\]', lambda m: m.group(0).replace('\n', ' '), full_text)

# Clean entry pattern
entry_pattern = re.compile(
    r'(?m)^([A-Za-z0-9][A-Za-z0-9\-\'\s/]{0,40}?)\s+'
    r'(?:\[([^\]\n]+)\]\s+)?'
    r'(n|v|adj|adv|prep|cj|num|pron|int|part|art|predic|attr|сокр)\b'
)

matches = list(entry_pattern.finditer(full_text))
print(f"Total entry matches: {len(matches)}")
...
print(f"Unique base headwords: {len(dict_by_base)}")
...
print(f"Direct match: {matched_direct} / {len(oxford_list)} ({matched_direct/len(oxford_list)*100:.2f}%)")
print(f"Unmatched count: {len(unmatched_direct)}")
