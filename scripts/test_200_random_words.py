...
print("=" * 70)
print("AUDIT: 200 RANDOM WORDS EVALUATION AGAINST MULLER 2021 PDF")
print("=" * 70)
...
print(f"Indexed {len(entry_index)} headwords in Muller index.")

# 3. SAMPLE 200 RANDOM WORDS
random.seed(42)
sampled_words = random.sample(oxford_data, 200)

print(f"Sampled 200 random words.")

# 4. EVALUATE EACH WORD
results = []
pass_count = 0
fail_count = 0
...
for item in sampled_words:
    ...
    # Check if word is in Muller
    if not muller_entries:
        # Check if it was a fallback word
    ...
    # Re-parse raw muller entries to get exact gold-standard meanings
    gold_meanings = []
    for me in muller_entries:
        gold_meanings.extend(parse_muller_body(word, me['body']))
        
    # Compare json_meanings vs gold_meanings
    issues = []
    ...
    if len(json_meanings) == 0:
        ...
        for i in range(min_len):
            ...
            # Check translation text similarity / equality
            # Allow minor differences from polish (e.g. removed spurious hyphen)
...
with open('scripts/evaluation_200_words_report.json', 'w', encoding='utf-8') as f:
    json.dump({
        'total': 200,
...
print("Saved evaluation report to scripts/evaluation_200_words_report.json")
