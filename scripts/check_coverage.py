...
for item in oxford_data:
    ...
    if w in index_by_hw:
        matched.append(w)
    elif w.replace('_', ' ') in index_by_hw:
        matched.append(w)
    elif w.replace(' ', '-') in index_by_hw:
        matched.append(w)
    elif w.replace('-', ' ') in index_by_hw:
        matched.append(w)
    else:
...
print(f"Matched directly: {len(matched)} / {len(oxford_data)} ({len(matched)/len(oxford_data)*100:.1f}%)")
print(f"Unmatched count: {len(unmatched)}")
print(f"Sample unmatched words (first 30): {unmatched[:30]}")
